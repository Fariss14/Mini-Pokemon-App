...
const Team = () => {
  const { team, removeFromTeam, randomizeTeam, loading } = usePokemon()
  ...
  const handleRandomizeTeam = async () => {
    const result = await randomizeTeam()
    setMessage(result.message)
    ...
    <div className="container">
      ...
      <div className="team-actions-top">
        <button onClick={handleRandomizeTeam} className="randomize-button" disabled={loading}>
          {loading ? "Loading..." : "Randomize Team"}
        </button>
      </div>

      {team.length === 0 ? (
        <div className="empty-team">
          <p>Your team is empty! Add some Pokémon from the Pokédex or use the randomize button.</p>
